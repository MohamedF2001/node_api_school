...
const searchProfesseurByMatiere = (req, res) => {
  ...
  db.query(searchSQL, [matiere_id], (err, results) => {
    if (err) {
      console.error("Erreur lors de la recherche des professeurs :", err);
      res.status(500).send("Erreur lors de la recherche des professeurs");
      return;
    }
