...
const alterTableSQL = `
  ALTER TABLE eleve ADD COLUMN matricule VARCHAR(50) UNIQUE NOT NULL;
`;

db.query(alterTableSQL, (err) => {
  if (err) {
    if (err.code === 'ER_DUP_FIELDNAME') {
      console.log("La colonne 'matricule' existe déjà.");
    } else {
      console.error("Erreur lors de la modification de la table :", err);
    }
  } else {
    console.log("Colonne 'matricule' ajoutée avec succès !");
  }
